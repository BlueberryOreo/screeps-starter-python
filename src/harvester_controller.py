...
def run_harvester(creep: Creep):
    ...
    elif not creep.memory.filling and creep.store.getCapacity() <= 0:
        creep.memory.filling = True
    
    if creep.memory.path == None:
        source = _.sample(creep.room.find(FIND_SOURCES))
        spawn = _.sample(creep.room.find(FIND_MY_SPAWNS))
        path = PathFinder.search(spawn, {"pos": source.pos, "range": 1})
        creep.memory.path = Room.serializePath(path.path)
